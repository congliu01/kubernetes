...
func (f *framework) RunPreFilterPlugins(
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, preFilter, &status)
...
func (f *framework) RunPreFilterExtensionAddPod(state *CycleState, podToSchedule *v1.Pod,
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, preFilterExtensionAddPod, &status)
	for _, pl := range f.preFilterPlugins {
...
func (f *framework) RunPreFilterExtensionRemovePod(state *CycleState, podToSchedule *v1.Pod,
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, preFilterExtensionRemovePod, &status)
	for _, pl := range f.preFilterPlugins {
		if pl.PreFilterExtensions() == nil {
			continue
		}
...
func (f *framework) RunFilterPlugins(state *CycleState,
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, filter, &status)
...
func (f *framework) RunPostFilterPlugins(
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, postFilter, &status)
...
func (f *framework) RunScorePlugins(state *CycleState, pod *v1.Pod, nodes []*v1.Node) (ps PluginToNodeScores, status *Status) {
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, score, &status)
...
func (f *framework) RunPreBindPlugins(
	...
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, preBind, &status)
...
func (f *framework) RunBindPlugins(state *CycleState, pod *v1.Pod, nodeName string) (status *Status) {
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, bind, &status)
...
func (f *framework) RunReservePlugins(
	state *CycleState, pod *v1.Pod, nodeName string) (status *Status) {
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, reserve, &status)
...
func (f *framework) RunPermitPlugins(
	state *CycleState, pod *v1.Pod, nodeName string) (status *Status) {
	startTime := time.Now()
	defer recordExtensionPointDuration(startTime, permit, &status)
	timeout := maxTimeout
	statusCode := Success
...
// status is a double pointer to Status because Status is not created when arguments
// of this function is evaluated in "defer".
